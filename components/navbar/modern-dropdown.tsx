...
import { _modern } from '../../_data/_menu';
import classNames from 'classnames';
import { ChevronDownIcon } from '@heroicons/react/20/solid';
import Link from 'next/link';
// import Image from 'next/image';

export default function ModernDropdown({ isScrolling, item }) {
  return (
                ...
                <div className="overflow-hidden rounded-b-md bg-base-body text-sm shadow-2xl">
                  <div
                    className={classNames(
                      ...
                      'px-4 pb-4 pt-4 overflow-hidden overflow-y-auto scrollbar-hide grid grid-cols-5 gap-0 shadow-inner'
                    )}
                  >
                    {_modern.map((item, index) => (
                      <Link
                        prefetch={true}
                        onClick={() => {
                          close();
