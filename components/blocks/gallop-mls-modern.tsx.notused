...
import MLSHomeMiniCard from '@components/mls/home-mini-card';
import { usePathname } from 'next/navigation';

export const GallopMLSModern = ({ className, meta, props, sort }) => {
  const [count, setCount] = useState<number>(10);
  const [mlsData, setMlsData] = useState<any[]>(
    sort === 'recent' ? meta.recent : meta.mls || []
  );
  ...
  const isMiniCard =
    meta.slug === 'modern' ||
    meta.slug === 'mid-century' ||
    meta.slug === 'dallas-modern-homes'
      ? true
      : false;
    ...
    <>
      <div
        className={classNames(
          ...
          isMiniCard ? 'mb-7 gap-7' : ' mb-14 gap-14'
        )}
      >
