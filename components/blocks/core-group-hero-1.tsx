    ...
    <Fragment>
      ...
      <div className="!max-w-none bg-secondary-main">
        <div className="flex flex-col xl:flex-row !max-w-screen-3xl px-4 sm:px-8 mx-auto gap-0">
          <div className="py-14 px-4 bg-primary-main text-primary-contrast w-4/12 -mt-20 relative z-30 rounded-t-md overflow-hidden">
            <div className="-z-10 bg-primary-main/90 absolute inset-0"></div>
            <h2 className="mb-2 leading-tight text-2xl md:text-3xl w-full text-center text-primary-contrast">
              {brand.h2.text}
            </h2>
          </div>
            ...
            {info &&
              ...
              info.wpBlockGroup.map((item: any, index: number) => {
                ...
                return (
                  <a
                    href={item.p?.a?.href}
                    key={`info-item-${index + 1}`}
                    className={classNames(
                      'py-20 px-12 text-white w-1/3 flex flex-col gap-10 shrink-0 hover:!bg-white/10'
                    )}
                    style={{
